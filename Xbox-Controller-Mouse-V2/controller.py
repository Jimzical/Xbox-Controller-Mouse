...
# +++++++++++++++++++++++++++++++Right Trigger cases++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      # ====================================================================================
            ...
            #Diagonal Right Up
            if(joy.read()['rightx'] > 0.90 and joy.read()['righty'] < -0.90):
                  gui.moveRel(15,15,0.00001) 
            #Diagonal Right Down
            if(joy.read()['rightx'] > 0.90 and joy.read()['righty'] > 0.90):
                  gui.moveRel(15,-15,0.00001) 
            #Diagonal Left Up
            ...
            if(joy.read()['rightx'] > 0.90):
                  gui.moveRel(15,0,0.00001) 
            # left
            if(joy.read()['rightx'] < -0.90):
                  gui.moveRel(-15,0,0.00001)
            # down
            if(joy.read()['righty'] > 0.90):
                  gui.moveRel(0,-15,0.00001) 
            # up
            if(joy.read()['righty'] < -0.90):
                  gui.moveRel(0,15,0.00001)
            ...
            if(joy.read()['rightx'] > 0.95):
                  gui.moveRel(65,0,0.00001) 
            # left
            if(joy.read()['rightx'] < -0.95):
                  gui.moveRel(-65,0,0.00001)
            # down
            if(joy.read()['righty'] > 0.95):
                  gui.moveRel(0,-65,0.00001) 
            # up
...
# +++++++++++++++++++++++++++++++Left Trigger cases++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      # Doesent work for some reason
            ...
            # right
            if(joy.read()['rightx'] > 0.80):
                  gui.dragRel(15,0,0.00001) 
            # left
            if(joy.read()['rightx'] < -0.80):
                  gui.dragRel(-15,0,0.00001)
            # down
            if(joy.read()['righty'] > 0.80):
                  gui.dragRel(0,-15) 
            # up
            if(joy.read()['righty'] < -0.80):
                  gui.dragRel(0,15)
